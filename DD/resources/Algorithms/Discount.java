...
public class Discount {
	...
	/**
	 * we suppose that car's batteryLevel belongs to a range between 0 and 100
	 */
	 ...
	 * @param charging is true if rental's car has been plugged to the power grid
	 */
	public double discountCharging(Set<SafeArea> map, boolean charging){
		if(charging==false) return 0;
		
		for(SafeArea a: map){
			if(a.getPosition().equals(rental.getCar().getPosition())==true 
				&& a.isPowerGridPresence()==true) return rental.getAmountToPay()*0.3;
		}
		
		return 0;
	}
	
	public void calculateDiscounts(int atLeastTwoPassengersTime, Set<SafeArea> map, boolean charging){
		double totalDiscount=this.discountPassengers(atLeastTwoPassengersTime)+
