...
\item If one-character variables are used, they are used only for temporary \enquote{throwaway} variables, such as those used in for loops.
...
\item Consistent bracing style is used, either the preferred \enquote{Allman} style (first brace goes underneath the opening block) or the \enquote{Kernighan and Ritchie} style (first brace is on the same line of the instruction that opens the new block).
...
\item Implementation avoids \enquote{brutish programming}. (See \url{http://users.csc.calpoly.edu/~jdalbey/SWE/CodeSmells/bonehead.html} for more information.) 
...
\item Check \texttt{throw-catch} expressions, and check that the error condition is actually legitimate.
